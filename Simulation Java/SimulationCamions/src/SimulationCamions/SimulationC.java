	...
	public static void main(String[] args) throws InterruptedException {
		...
		while(true)
		{
			...
			for(int i = 0; i < listCamion.size();i++)
			{
				...
				if(listCamion.get(i).getCoordonneeDepart().equals(listCamion.get(i).getCoordonneeDestination()) && 
						listCamion.get(i).getStatut() == statutCamion.valueOf("retour"))
				{
					camionsSimuCamion.updateInsertDeleteCamion(bankRequeteLoginBDD.getRequeteSupressionCamion()+" WHERE idCamion =", listCamion.get(i));
				}
			}
